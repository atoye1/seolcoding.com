 ...
 * 1. WikiLinks [[Page Name]] → [Page Name](page-name.html)
 * 2. Image WikiLinks ![[image.png]] → ![](../images/image.png)
 * 3. Attachment links [[file.pdf]] → [file.pdf](../images/file.pdf)
 * 4. Hashtags #tag → Front Matter tags: [tag]
 * 5. Date format standardization
 * 6. File name normalization (spaces → hyphens)
 *
 * USAGE:
...
function convertWikiLinks(content) {
  // Convert WikiLinks with custom text: [[Page Name|Custom Text]] → [Custom Text](page-name.html)
  content = content.replace(/\[\[([^\]|]+)\|([^\]]+)\]\]/g, (match, link, text) => {
    const slug = slugify(link);
    return `[${text}](../${slug}/)`;
  });

  // Convert simple WikiLinks: [[Page Name]] → [Page Name](page-name.html)
...
function convertImageLinks(content) {
  // Convert image WikiLinks: ![[image.png]] → ![](../images/image.png)
...
function convertYouTubeLinks(content) {
  // Convert YouTube image syntax: ![](https://www.youtube.com/watch?v=VIDEO_ID) → {% youtube VIDEO_ID %}
  content = content.replace(/!\[\]\((https?:\/\/(?:www\.)?youtube\.com\/watch\?v=([a-zA-Z0-9_-]+))\)/g, (match, fullUrl, videoId) => {
    return `{% youtube ${videoId} %}`;
  });

  // Convert YouTube image syntax: ![](https://youtu.be/VIDEO_ID) → {% youtube VIDEO_ID %}
  content = content.replace(/!\[\]\((https?:\/\/youtu\.be\/([a-zA-Z0-9_-]+))\)/g, (match, fullUrl, videoId) => {
    return `{% youtube ${videoId} %}`;
  });

  // Convert standard YouTube links: [text](https://www.youtube.com/watch?v=VIDEO_ID) → {% youtube VIDEO_ID %}
...
function convertAttachmentLinks(content) {
  // Convert attachment WikiLinks for non-image files: [[document.pdf]] → [document.pdf](../images/document.pdf)
...
function processDirectory(inputDir, outputDir) {
  ...
  files.forEach(file => {
    ...
    if (stat.isDirectory()) {
      ...
      if (file === 'attachments') {
        console.log(`⏭️  Skipping attachments directory: ${file}`);
        return;
      }
...
// Main execution
console.log('🚀 Starting Obsidian to Hexo conversion...');
